...
import type { Route, SiteRoute, SiteTrain, SiteStation, Delay, RouteStation } from "../types";
...
const stationDelay = (station: RouteStation): Delay => {
  const delay: Delay = { arrival: false, departure: false };
...
const dayAndTime = (str: string, tz: string): string => {
...
const TAG_BG_COLORS = new Map<string, string>([
...
const TAG_TEXT_COLORS = new Map<string, string>([
...
const STATION_COLORS = new Map<string, string>([
  ["arrived", "bg-gold"],
  ["enroute", "bg-green"],
  ["departed", "bg-primary-dark"],
...
async function buildSite(): Promise<void> {
  // Get the route data
  const routes: SiteRoute[] = (JSON.parse(
    await fs.readFile("_site/routes.json", { encoding: "utf-8" }),
  ) as Route[]).map((routeInfo) => {
    ...
    route.trains = routeInfo.trains.map((trainInfo) => {
      const train: SiteTrain = {
        ...
        tag: { text: "scheduled", bg: "bg-base-lighter", color: "text-black" },
        stations: [],
        from: "",
        to: "",
      };
      ...
      train.stations = trainInfo.stations.map((stationInfo) => {
        const station: SiteStation = {
          ...
          tag: {
            ...
            bg: TAG_BG_COLORS.get(stationInfo.status) ?? "bg-base-light",
            color: TAG_TEXT_COLORS.get(stationInfo.status) ?? "text-black",
          },

          dotColor: STATION_COLORS.get(stationInfo.status) ?? "bg-base-light",
        ...
        if (station.status === "enroute" || station.status === "scheduled") {
          ...
          if (station.delay.arrival || station.delay.departure) {
            info.push("This train is running behind schedule.");
      ...
      if (train.stations[train.stations.length - 1].status === "arrived") {
        train.status = "arrived";
      }
      train.tag = {
        text: train.status,
        bg: TAG_BG_COLORS.get(train.status) ?? "bg-base-light",
        color: TAG_TEXT_COLORS.get(train.status) ?? "text-black",
      };
      ...
      if (train.status === "scheduled") {
        ...
        const { previous, next } = (() => {
          const index = train.stations.findLastIndex(
            (station: SiteStation) => station.status === "departed" || station.status === "arrived",
          );
          if (index > 0) {
            return {
        ...
        if (previous?.delay.departure || next?.delay.arrival) {
          ...
          train.tag.bg = TAG_BG_COLORS.get("delayed") ?? "bg-secondary-dark";
          train.tag.color = TAG_TEXT_COLORS.get("delayed") ?? "text-white";
        }
      ...
      train.to = train.stations[train.stations.length - 1].name;
